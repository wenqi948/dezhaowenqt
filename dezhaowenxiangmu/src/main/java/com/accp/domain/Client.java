package com.accp.domain;

import java.util.Date;

import com.fasterxml.jackson.annotation.JsonFormat;

public class Client {
    private String clientno;

    private Integer mid;

    private String artisanid;

    private String clientname;

    private String clientlxr;

    private String clientphone;

    private String clientaddresss;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date clientday;

    private Integer clientzq;

    private Integer clientmoney;

    private String clientgw;

    private String clientgwphone;

    private String clientbz;

    private String yuliu;

    private String yuliu2;

    private String yuliu3;

    public String getClientno() {
        return clientno;
    }

    public void setClientno(String clientno) {
        this.clientno = clientno;
    }

    public Integer getMid() {
        return mid;
    }

    public void setMid(Integer mid) {
        this.mid = mid;
    }

    public String getArtisanid() {
        return artisanid;
    }

    public void setArtisanid(String artisanid) {
        this.artisanid = artisanid;
    }

    public String getClientname() {
        return clientname;
    }

    public void setClientname(String clientname) {
        this.clientname = clientname;
    }

    public String getClientlxr() {
        return clientlxr;
    }

    public void setClientlxr(String clientlxr) {
        this.clientlxr = clientlxr;
    }

    public String getClientphone() {
        return clientphone;
    }

    public void setClientphone(String clientphone) {
        this.clientphone = clientphone;
    }

    public String getClientaddresss() {
        return clientaddresss;
    }

    public void setClientaddresss(String clientaddresss) {
        this.clientaddresss = clientaddresss;
    }

    public Date getClientday() {
        return clientday;
    }

    public void setClientday(Date clientday) {
        this.clientday = clientday;
    }

    public Integer getClientzq() {
        return clientzq;
    }

    public void setClientzq(Integer clientzq) {
        this.clientzq = clientzq;
    }

    public Integer getClientmoney() {
        return clientmoney;
    }

    public void setClientmoney(Integer clientmoney) {
        this.clientmoney = clientmoney;
    }

    public String getClientgw() {
        return clientgw;
    }

    public void setClientgw(String clientgw) {
        this.clientgw = clientgw;
    }

    public String getClientgwphone() {
        return clientgwphone;
    }

    public void setClientgwphone(String clientgwphone) {
        this.clientgwphone = clientgwphone;
    }

    public String getClientbz() {
        return clientbz;
    }

    public void setClientbz(String clientbz) {
        this.clientbz = clientbz;
    }

    public String getYuliu() {
        return yuliu;
    }

    public void setYuliu(String yuliu) {
        this.yuliu = yuliu;
    }

    public String getYuliu2() {
        return yuliu2;
    }

    public void setYuliu2(String yuliu2) {
        this.yuliu2 = yuliu2;
    }

    public String getYuliu3() {
        return yuliu3;
    }

    public void setYuliu3(String yuliu3) {
        this.yuliu3 = yuliu3;
    }
}