package com.accp.domain;

import java.util.Date;

import com.fasterxml.jackson.annotation.JsonFormat;

public class Clientcar {
	private Client client;
    public Client getClient() {
		return client;
	}

	public void setClient(Client client) {
		this.client = client;
	}

	private String carno;

    private String typeid;

    private String people;

    private String peoplephone;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date peopleday;

    private String carguishu;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date cardaoqi;

    private String chejiahao;

    private String fadongji;

    private Integer carniankuan;

    private Integer licheng;

    private String fadongjipp;

    private Float zaizhong;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date buyday;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date shangpaiday;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date chejian;

    private String ranyou;

    private String chexi;

    private String jiaoqiang;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date jiaoqiangday;

    private String shangye;
    @JsonFormat(pattern="yyyy-MM-dd")
    private Date shangyeday;

    private String yuliu;

    private String yuliu2;

    private String yuliu3;

    public String getCarno() {
        return carno;
    }

    public void setCarno(String carno) {
        this.carno = carno;
    }

    public String getTypeid() {
        return typeid;
    }

    public void setTypeid(String typeid) {
        this.typeid = typeid;
    }

    public String getPeople() {
        return people;
    }

    public void setPeople(String people) {
        this.people = people;
    }

    public String getPeoplephone() {
        return peoplephone;
    }

    public void setPeoplephone(String peoplephone) {
        this.peoplephone = peoplephone;
    }

    public Date getPeopleday() {
        return peopleday;
    }

    public void setPeopleday(Date peopleday) {
        this.peopleday = peopleday;
    }

    public String getCarguishu() {
        return carguishu;
    }

    public void setCarguishu(String carguishu) {
        this.carguishu = carguishu;
    }

    public Date getCardaoqi() {
        return cardaoqi;
    }

    public void setCardaoqi(Date cardaoqi) {
        this.cardaoqi = cardaoqi;
    }

    public String getChejiahao() {
        return chejiahao;
    }

    public void setChejiahao(String chejiahao) {
        this.chejiahao = chejiahao;
    }

    public String getFadongji() {
        return fadongji;
    }

    public void setFadongji(String fadongji) {
        this.fadongji = fadongji;
    }

    public Integer getCarniankuan() {
        return carniankuan;
    }

    public void setCarniankuan(Integer carniankuan) {
        this.carniankuan = carniankuan;
    }

    public Integer getLicheng() {
        return licheng;
    }

    public void setLicheng(Integer licheng) {
        this.licheng = licheng;
    }

    public String getFadongjipp() {
        return fadongjipp;
    }

    public void setFadongjipp(String fadongjipp) {
        this.fadongjipp = fadongjipp;
    }

    public Float getZaizhong() {
        return zaizhong;
    }

    public void setZaizhong(Float zaizhong) {
        this.zaizhong = zaizhong;
    }

    public Date getBuyday() {
        return buyday;
    }

    public void setBuyday(Date buyday) {
        this.buyday = buyday;
    }

    public Date getShangpaiday() {
        return shangpaiday;
    }

    public void setShangpaiday(Date shangpaiday) {
        this.shangpaiday = shangpaiday;
    }

    public Date getChejian() {
        return chejian;
    }

    public void setChejian(Date chejian) {
        this.chejian = chejian;
    }

    public String getRanyou() {
        return ranyou;
    }

    public void setRanyou(String ranyou) {
        this.ranyou = ranyou;
    }

    public String getChexi() {
        return chexi;
    }

    public void setChexi(String chexi) {
        this.chexi = chexi;
    }

    public String getJiaoqiang() {
        return jiaoqiang;
    }

    public void setJiaoqiang(String jiaoqiang) {
        this.jiaoqiang = jiaoqiang;
    }

    public Date getJiaoqiangday() {
        return jiaoqiangday;
    }

    public void setJiaoqiangday(Date jiaoqiangday) {
        this.jiaoqiangday = jiaoqiangday;
    }

    public String getShangye() {
        return shangye;
    }

    public void setShangye(String shangye) {
        this.shangye = shangye;
    }

    public Date getShangyeday() {
        return shangyeday;
    }

    public void setShangyeday(Date shangyeday) {
        this.shangyeday = shangyeday;
    }

    public String getYuliu() {
        return yuliu;
    }

    public void setYuliu(String yuliu) {
        this.yuliu = yuliu;
    }

    public String getYuliu2() {
        return yuliu2;
    }

    public void setYuliu2(String yuliu2) {
        this.yuliu2 = yuliu2;
    }

    public String getYuliu3() {
        return yuliu3;
    }

    public void setYuliu3(String yuliu3) {
        this.yuliu3 = yuliu3;
    }
}